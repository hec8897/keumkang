...
// 이미지가 웹에서 보여질때 사용되어질 기본 URL입니다. // 웹루트 부터의 절대 URL을 입력합니다. 
...
$url = '' ;
 ...
 if (isset($_FILES['upload'])) { 
    ...
    $exif = exif_read_data($name);
     move_uploaded_file($source, $uploadfullPath . $name); 
 ...
 $Login_info = json_encode(
    array('filename'=> $name,"uploaded" => 1 ,"url"=> $url,'test'=>123,'test2'=>$exif )
);
...
//  window.parent.CKEDITOR.tools.callFunction('1', '$url', '$message')</script>"; 
 ?>